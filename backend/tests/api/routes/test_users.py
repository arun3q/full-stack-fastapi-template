...
async def test_update_password_me(
    ...
    verified, _ = verify_password(new_password, user_db.hashed_password or "")
    assert verified
    ...
    verified, _ = verify_password(
        settings.FIRST_SUPERUSER_PASSWORD, user_db.hashed_password or ""
    )
    assert verified
...
async def test_register_user(client: TestClient, db: AsyncSession) -> None:
    ...
    assert user_db.full_name == full_name
    verified, _ = verify_password(password, user_db.hashed_password or "")
    assert verified
