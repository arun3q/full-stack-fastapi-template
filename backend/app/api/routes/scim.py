...
from app.models import ORG_ROLE_MEMBER, ApiKey, Organization, User, UserCreate
...
def _user_resource(user: User, role: str | None = None) -> dict[str, Any]:
    return {
...
async def list_scim_users(
    ...
    for membership in memberships:
        ...
        if user is not None:
            resources.append(_user_resource(user, membership.role))
...
async def get_scim_user(
    ...
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")
    return _user_resource(user, membership.role)
...
async def create_scim_user(
    ...
    if existing:
        ...
        return _user_resource(existing, ORG_ROLE_MEMBER)
    ...
    await session.commit()
    return _user_resource(user, ORG_ROLE_MEMBER)
